...
export default function Cart() {
    ...
    const handleQuanity = (e, product, size) =>{
        cart.products.map((element, idx)=>{
            console.log(element, product)
            if(element.productDetails._id == product._id && (!element.size || element.size == size )){
    ...
    <div id="Cart">
        <section id="page-header" className="about-header" style={{backgroundImage : `url(${PngIcons.banner.banner8})`}}>
                ...
                <tbody>
                    {
                        cartData && cartData.products && cartData.products.map((element, idx)=>(
                            <tr>
                            <td><a href="#" onClick={()=> {removeProduct(element.productDetails, element.size)}}><i className="far fa-times-circle" /></a></td>
                            <td><img src={element?.productDetails?.images[0]} alt /></td>
                            <td>{element.productDetails.name}</td>
                            <td>{element.size || "-"}</td>
                            <td>{element.productDetails.price}</td>
                            <td><input type="number" defaultValue={element.quantity} name id onChange={(e)=>{handleQuanity(e, element.productDetails, element.size)}} /></td>
            ...
            <section id="cart-add" className="section-p1">
                ...
                <button className="normal" onClick={handleCheckout}>{loader ? <CircularProgress size={20} color={"inherit"} /> : "Proceed to checkout"}</button>
                </div>
            </section>
    </div>
  )
